...
import { buildPassportAidSign, normalizePassportPath, passportNoonUtcTs } from '../sign/aid-sign'
import {
  CREATOR_AID,
  CREATOR_ORIGIN,
...
import { randomBizTraceId, randomDesktopHex } from '../sign/sign-qs'
...
export class DouyinHttp {
  ...
  async requestRaw (url: string, init: RequestInit = {}): Promise<HttpResponse<string>> {
    ...
    logger.debug(`[douyin:http] ${(init.method ?? 'GET').toUpperCase()} ${res.status} ${path}${rawText ? ` 响应: ${rawText.length > 5000 ? `${rawText.slice(0, 5000)}...(截断,共${rawText.length}字符)` : rawText}` : '(空)'}`)
