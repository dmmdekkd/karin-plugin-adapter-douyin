...
import {
  encodeDesktopPassportParams,
  randomDesktopHex,
} from '../sign/sign-qs'
import type { DouyinHttp } from '../http/client'
import type { CheckQrconnectData } from './types'
import { DESKTOP_APP_VERSION, DESKTOP_LITE_AID, desktopLitePassportFormPost } from './passport-lite'
 ...
 * 跳过 is_login / null。
 */
async function packVerifyWaysData (
  http: DouyinHttp,
  decision: Record<string, unknown>,
  deviceId: string,
...
async function loadVerifyCenterSetting (
  ...
  try {
    ...
    const res = await requestVerificationRaw(
      http,
      `https://vcs.zijieapi.com/vc/setting?${query}`,
      { method: 'GET', headers: { 'X-Setting-Flag': '1' } },
    )
