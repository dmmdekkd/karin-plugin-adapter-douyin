import { DEFAULT_BROWSER_INFO, encodeBrowserInfo } from '../sign/browser-info'
import { mixModeEncode } from '../sign/mix-mode'
import { DESKTOP_PASSPORT_APP_KEY } from '../sign/constants'
import {
  buildDesktopPassportBaseQuery,
  encodeFormBody,
  desktopPassportUrl,
  signPassportQuery,
  type SignPassportExtras,
} from '../sign/sign-qs'
import type { DouyinHttp } from '../http/client'
import { DESKTOP_LITE_AID, desktopLitePassportFormPost } from './passport-lite'
import {
  parseVerificationDecision,
  runBrowserVerification,
  stringifyVerificationFields,
} from './verification'
import type {
  CheckQrconnectData,
...
export async function validateQrMfaCode (http: DouyinHttp, challenge: QrMfaChallenge, verifyWay: string, code?: string): Promise<QrMfaResponse> {
  ...
  const encoded = code == null
    ? undefined
    : verifyWay === 'mobile_sms_verify' ? mixModeEncode(code) : codeEncrypt(code)
